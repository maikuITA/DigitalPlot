...
class VProfile {
     ...
     * @return void
     */
    public static function render(string $user, int $plotPoints, $proPic = null, bool $isLogged = false, int $privilege = BASIC ): void {
    ...
    public static function showArticlesWriter(string $user, int $plotPoints, $proPic = null, bool $isLogged = false, int $privilege = BASIC, $articles = []){
        $smarty = StartSmarty::configuration();
        ULogSys::toLog("Display -> profilo.tpl");
        $smarty->assign('user', $user);
        $smarty->assign('isLogged', $isLogged);
